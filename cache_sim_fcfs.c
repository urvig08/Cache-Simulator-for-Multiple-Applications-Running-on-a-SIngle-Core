...
typedef struct {
    ...
    int tag;
    int fifo_counter; 
                                /* This fifo counter is only used for fully associative cache. It indicates the "oldest" block in the cache 
...
void execute_dm (mem_access_t access, int block_number, int index_size, cache_t * f_cache) {
    int cutOffset = (access.address >> ((int)log2(block_size))) % (block_number);     // extracting the offset
    int tag = (access.address >> (((int)log2(block_size)) + index_size));             // extracting the size of the tag 
    int f_counter = 0;                                          // initialising the counters for total accesses and total number of hits
    int f_hit_counter = 0;

    for (int i= 0; i < block_number; i++) {                     // search the total cache
        if (i == cutOffset) {
            f_counter++;
            if (tag == f_cache->block_array[i].tag) {           // the tag is found in the cache 
                if (f_cache->block_array[i].valid_bit == 1) {   // valid bit is equal to 1
                    f_hit_counter++;                            // increment the counter for hit
                } else {                                        // valid bit is not equal to 1
                    f_cache->block_array[i].tag = tag;          // update the tag
                    f_cache->block_array[i].valid_bit = 1;      // update the valid bit to 1
                }
...
void execute_fa (mem_access_t access, int block_number, cache_t * f_cache) {
    ...
    for (int i= 0; i< block_number; i++) {                                                      // searching in cache
        if (tag == f_cache->block_array[i].tag && f_cache->block_array[i].valid_bit == 1) {     // the tag is found in the cache and the valid bit is equal to 1
            ...
            f_cache->block_array[i].tag = tag;                              // update the tag
            f_cache->block_array[i].valid_bit = 1;                          // update the valid bit to 1
            f_cache->block_array[i].fifo_counter = 0;                       // update fifo_counter to 0 
            found = 1;
            x = i + 1;
            f_counter++;
            break;
...
void main(int argc, char** argv) {
    ...
    if ( argc != 5 ) { 
        printf("Usage: ./cache_sim_fcfs [cache size: 32K] [block size: 8|32|128] [cache mapping: dm|fa] [cache organization: uc]\n");
        exit(0);
    } else  {
    ...
    ptr_file1 =fopen("ccompiler.trace","r");
    if (!ptr_file1) {
        printf("Unable to open the trace file\n");
        exit(1);
    }

    ptr_file2 =fopen("spice.trace","r");
    if (!ptr_file2) {
        printf("Unable to open the trace file\n");
        exit(1);
    }
    
    ptr_file3 =fopen("trace.din","r");
    if (!ptr_file3) {
        printf("Unable to open the trace file\n");
        exit(1);
    }
